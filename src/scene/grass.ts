...
import { makeTuftTexture } from '../textures';
...
export function buildGrass(scene: THREE_T.Scene, THREE: typeof THREE_T, opts: BuildGrassOptions): GrassTuft[] {
    const { cols, rows, cell } = opts;
    const count = opts.count ?? 280;
    const spread = opts.spread ?? 1.3;
    const textureSize = opts.textureSize ?? 128;

    const tuftTexture = makeTuftTexture(textureSize);
