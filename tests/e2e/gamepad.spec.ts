...
test.describe('gamepad detection', () => {
    test('ghost (connected=false) pad is ignored — regression for iOS/macOS Safari', async ({ page }) => {
        await stubGamepads(page, [{ id: 'Xbox Wireless Controller', connected: false }, null, null, null]);
    ...
    test('connected gamepad activates gamepad prompt', async ({ page }) => {
        await stubGamepads(page, [{ id: 'Xbox 360 Controller (XInput STANDARD GAMEPAD)', connected: true }]);
        await gotoGame(page, { lang: 'en-us' });
