...
test.describe('timer', () => {
  test('does not advance before the first directional input', async ({ page }) => {
    ...
    // until the user actually presses an arrow key.
  ...
  test('advances only while the game is unpaused', async ({ page }) => {
    ...
    await page.waitForTimeout(1700);                    // wait while paused
...
test.describe('auto-pause on focus loss', () => {
  test('window blur pauses an active game', async ({ page }) => {
    ...
    expect(await page.evaluate(() => (window as any).__test.state().paused)).toBe(false);

    await page.evaluate(() => window.dispatchEvent(new Event('blur')));
    await page.waitForTimeout(100);

    expect(await page.evaluate(() => (window as any).__test.state().paused)).toBe(true);
